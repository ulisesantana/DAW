  ...
  // Los métodos del controlador se diseñan pensandos
	...
	public function insertarIncidencia (Request $request){
		...
		if (empty($request->asunto)) {
			return view('incidencias/formularioRegistrarIncidencia', ['mensaje' => "Debe rellenar los campos obligatorios de la incidencia"]);
		} else {
			...
			return view('incidencias/formularioRegistrarIncidencia', ['mensaje' => "Incidencia insertada correctamente"]);
		}
	}

	public function listarIncidencias () {
		if (!file_exists("incidencias.dat")){
			return view('incidencias/formularioRegistrarIncidencia', ['mensaje' => "No existen incidencias registradas"]);
		} else {
			// Cargamos el Modelo:
			...
			return view('incidencias/listadoIncidencias', ['incidencias' => $arrayIncidencias]);
		}
	}
	...
	public function buscarIncidencias (Request $request){
		// validación de los datos recibidos:
		if (empty($request->asunto)) {
			return view('incidencias/formularioRegistrarIncidencia', ['mensaje' => "Debe indicar el asunto por el que buscar"]);
		} else {
			if (!file_exists("incidencias.dat")){
				return view('incidencias/formularioRegistrarIncidencia', ['mensaje' => "No existen incidencias registradas"]);
			} else {
